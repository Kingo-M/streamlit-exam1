                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        if "main_page_number" in message["content"]:
                            st.success(f"{message['content']['main_file_path']}(ページ番号:{message['content']['main_page_number']})", icon=icon)
                        else:
                            st.success(f"{message['content']['main_file_path']}(ページ番号:不明)", icon=icon)
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    ...
                                    st.info(f"{sub_choice['source']}(ページ番号:不明)", icon=icon)
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        if "page" in llm_response["context"][0].metadata:
            ...
            st.success(f"{main_file_path}(ページ番号:不明)", icon=icon)
...
def display_sidebar():
    with st.sidebar:
        display_select_mode()
        display_initial_ai_message_guide()

def display_initial_ai_message_guide():#問題3への回答：サイドバー表示メッセージ
    with st.chat_message("assistant"):
        # 「社内文書検索」の機能説明
